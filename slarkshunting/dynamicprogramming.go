...
An Explorer is looking for Slarks but Boojums make people dissapear from the earth.

Exercise to resolve by using Dynamic Programming.

Having a nxn 2-d slice that shows 1 when the position has an Slark,
 -2 when it has a Boojum and 0 when it doesn't have neither and Slark nor an Boojum.

 What is the maximum quantity of Slark we can found without meeting a Boojum having in count:
...
func slarkHunting() {
	...
	// the result of maximum quantity of slark found is in the last position of the 2D slice
	H := generateDP(M)
	fmt.Printf("Slark hunted: %v\n", H[len(H)-1][len(H[0])-1].sum)
...
func generateDP(M [][]int) [][]obj {
	...
	for r := 1; r < 9; r++ {
		...
		for c := 1; c < 9; c++ {
			...
			// si los dos precedentes son -1
			if H[r-1][c].v == -1 && H[r][c-1].v == -1 { // can unify this **
				H[r][c].v = -1
